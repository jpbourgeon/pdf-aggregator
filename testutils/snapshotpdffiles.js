...
const encode = async (folder) => {
  let pdfFiles = await fs.readdir(folder)
    .catch(e => console.log(`snapshotPdfFiles > encode fs.readdir: ${e.message}`)); // eslint-disable-line no-console
  ...
  const promises = pdfFiles.map(async (pdf) => {
    let content = await fs.readFile(`${folder}/${pdf}`)
      .catch(e => console.log(`snapshotPdfFiles > encode fs.readFile: ${e.message}`)); // eslint-disable-line no-console
    content = content
      .toString()
  ...
  const snapshot = await Promise.all(promises)
    .catch(e => console.log(`snapshotPdfFiles > encode Promise.all: ${e.message}`)); // eslint-disable-line no-console

  return snapshot.sort();
};

const hash = async (folder) => {
  let pdfFiles = await fs.readdir(folder)
